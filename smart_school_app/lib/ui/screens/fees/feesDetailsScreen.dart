...
class FeesDetailsScreenState extends State<FeesDetailsScreen> {
  ...
  void initState() {
    ...
    if ((context
      ...
      payStackPlugin.initialize(
          publicKey: (context.read<AppConfigurationCubit>().getFeesSettings())
              .paystackPublicKey);
    }
  ...
  Future<void> payWithPayStack({
    ...
    required int amount,
    required Map paymentGatewayDetails,
  }) async {
    try {
      Charge charge = Charge()
        ..amount = amount
        ..reference = paymentTransactionId.toString()
        ..email = context.read<AuthCubit>().getParentDetails().email
        ..putMetaData("payment_gateway_details", paymentGatewayDetails)
        ..currency = context
            .read<AppConfigurationCubit>()
            .getFeesSettings()
            .currencyCode
